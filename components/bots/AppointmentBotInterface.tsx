...
interface AppointmentBotInterfaceProps {
    config: UserConfiguration;
}

const AppointmentBotInterface: React.FC<AppointmentBotInterfaceProps> = ({ config }) => {
    const [messages, setMessages] = useState<{ text: string; sender: 'user' | 'ai' }[]>([
        { text: `¡Hola! Bienvenido a ${config.businessName}. Puedo ayudarte a reservar una cita. Nuestros servicios son: ${config.services}. ¿Qué te gustaría agendar?`, sender: 'ai' }
    ]);
    ...
    const handleSend = async () => {
        ...
        let responseText = 'No he podido procesar tu solicitud.';

        if (aiResponse.functionCall) {
            const { service, date, time } = aiResponse.functionCall.args;
            responseText = `¡Perfecto! Hemos agendado tu cita para el servicio de "${service}" para el día ${date} a las ${time}. ¡Te esperamos!`;
        } else if (aiResponse.text) {
            responseText = aiResponse.text;
        }

        const aiMessage = { text: responseText, sender: 'ai' as const };
             ...
             <div className="p-4 border-t bg-white rounded-b-lg">
                <div className="flex items-center space-x-2">
                    <input
                        ...
                        placeholder="Ej: 'Quiero una cita para corte de pelo mañana a las 5pm'"
...
export default AppointmentBotInterface;