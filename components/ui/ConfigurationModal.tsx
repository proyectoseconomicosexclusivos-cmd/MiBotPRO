...
interface ConfigurationModalProps {
  bot: BotTemplate;
  onClose: () => void;
  onSave: (configData: FormData) => void;
...
const ConfigurationModal: React.FC<ConfigurationModalProps> = ({ bot, onClose, onSave, existingConfig }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (existingConfig) {
      // For edits, we just pass the updated data up to the parent component (Dashboard)
      onSave(formData);
    } else {
      // For new bots, we add it using the context function
      appContext?.addUserConfiguration({
          templateId: bot.id,
          templateTitle: bot.title,
          ...formData,
      });
      // Then we call the onSave callback, which will handle modal closing and navigation
      onSave(formData);
    }
